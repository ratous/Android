...
public class BdAdapter {
    ...
    public BdAdapter(Context context){
        ...
        bdArticles = new CreateBdEchantillon(context, NOM_BDD, null,
                VERSION_BDD);
    }
    //si la base n’existe pas, l’objet SQLiteOpenHelper exécute la méthode
    // onCreate
    // si la version de la base a changé, la méthode onUpgrade sera lancée, dans
    //les 2 cas l’appel
    ...
    public long insererEchantillon(Echantillon unEchant){
        ...
        // colonne où on veut mettre la valeur)
    ...
    private Echantillon cursorToEchant(Cursor c){
        ...
        if (c.getCount() != 0){
            ...
            unEchant = new Echantillon(); //On créé un echantillon
    ...
    public Echantillon getEchantillonWithLib(String unCode){
        ...
        Cursor c = db.query(TABLE_ECHANT, new String[] {COL_ID,COL_CODE,
                        COL_LIB,
                        COL_STOCK}, COL_CODE + " LIKE \"" + unCode +"\"", null, null,
                null, null);
        return cursorToEchant(c);
    }
    public int updateEchantillon(String unCode, String unLibelle , Echantillon unEchant){
        //La mise à jour d'un echantillon dans la BDD fonctionne plus ou moins
        // comme une insertion,
        // il faut simple préciser quel echantillon on doit mettre à jour grâce
        // à son code
        ContentValues values = new ContentValues();
        values.put(COL_CODE, unEchant.getCode());
        values.put(COL_LIB, unEchant.getLibelle());
        values.put(COL_STOCK, unEchant.getQuantiteStock());
        return db.update(TABLE_ECHANT, values, COL_CODE + " = \"" +unCode+"\"",
                null);
    }
